 ...
 *
 * Displays a clickable icon before to a track within a release page.
...
rl.ready(() => {
    ...
    function bindEvents(e) {
        ...
        let trackElem = e.currentTarget.parentElement.querySelector('span[class*="trackTitle"]');
        let trackName = '';

        trackName = sanitizeString(trackElem.textContent);

        const artistElem = e.currentTarget.parentElement.querySelector('td[class*="artist"]');
        let artistName = sanitizeString(artistElem.textContent);

        if (artistName === null || artistName === '') {
            artistName = sanitizeString(pageTitle.split('–')[0]);
        }
    ...
    function popIframe(ytId) {
        ...
        content = document.getElementById('site_footer_wrap').previousSibling;
        content.insertAdjacentHTML('beforebegin', iframe);
    }

    let ytApiKey = rl.getItem('userPreferences').ytApiKey;

    document.querySelectorAll('table[class*="tracklist"] tr').forEach(track => {
        let html = `
            <td class="enhance_track_preview">
                <a href="javascript:void(0);">
                    <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="20" height="20" x="0px" y="0px" viewBox="0 0 100 100" enable-background="new 0 0 100 100" xml:space="preserve">
